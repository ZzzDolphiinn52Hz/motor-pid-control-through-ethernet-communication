...
\hypersetup{
 ...
 filecolor=magenta, 
 urlcolor=cyan,
  pdftitle={Báo cáo thiết kế hệ thống điều khiển động cơ DC qua Ethernet},
...
THIẾT KẾ CARD ESP32 GIÁM SÁT VÀ ĐIỀU KHIỂN THỜI GIAN THỰC TỐC ĐỘ
ĐỘNG CƠ DC QUA ETHERNET VỚI GIAO DIỆN PYTHON
\par}
...
Đề tài xây dựng hệ thống giám sát và điều khiển tốc độ động cơ DC có encoder qua Ethernet. ESP32 đóng vai trò TCP Server, tự đọc encoder, tự chạy vòng điều khiển PI và xuất PWM điều khiển motor. Máy tính chạy giao diện Python Tkinter đóng vai trò HMI để gửi tham số, bật/tắt PI, điều khiển PWM thủ công và đọc telemetry. Dữ liệu giữa hai phía được đóng gói theo frame cố định 12 byte với header \texttt{@SEN} cho lệnh gửi xuống và \texttt{@REC} cho phản hồi.
...
Các mục tiêu cụ thể:
\begin{itemize}
    \item Thiết kế phần cứng sử dụng ESP32, module Ethernet W5500, driver L298N và động cơ DC có encoder.
    \item Cấu hình ESP32 giao tiếp với W5500 thông qua SPI và hoạt động như TCP Server trong mạng LAN.
    \item Thiết kế giao thức truyền thông ứng dụng dạng frame 12 byte, có kiểm tra lỗi bằng checksum XOR.
    \item Xây dựng firmware hai core: Core 0 xử lý TCP, Core 1 thực hiện vòng điều khiển realtime.
    \item Đọc encoder bằng PCNT, tính tốc độ RPM và điều khiển PWM bằng LEDC.
    \item Triển khai bộ điều khiển PI trên ESP32 để điều khiển tốc độ motor theo target RPM.
    \item Xây dựng giao diện Python Tkinter để kết nối, gửi lệnh, đọc telemetry, ghi log và vẽ biểu đồ realtime.
\end{itemize}
...
\begin{itemize}
    \item Truyền thông Ethernet TCP/IP trong mạng LAN.
    \item Giao tiếp SPI giữa ESP32 và W5500.
    \item TCP Server trên ESP32 và TCP Client trên Python.
    \item Thiết kế frame lệnh/phản hồi 12 byte.
    \item Điều khiển PWM, đọc encoder và giám sát tốc độ động cơ DC.
    \item Bộ điều khiển PI chạy cục bộ trong firmware ESP32.
\end{itemize}
...
\section{Cấu trúc tổng quan hệ thống}
...
\centering
\textbf{Python Tkinter GUI}\\
Kết nối TCP, gửi target/Kp/Ki, đọc RPM/PWM, vẽ biểu đồ, ghi log

\vspace{0.25cm}
\texttt{@SEN/@REC} qua TCP/IP Ethernet
\vspace{0.25cm}

\textbf{ESP32 + W5500}\\
Core 0: TCP Server \quad -- \quad Core 1: Timer, encoder, PI, PWM

\vspace{0.25cm}
\textbf{L298N + DC Motor + Encoder}
\end{minipage}}
\end{center}
...
\section{Ethernet và module W5500}
...
\begin{tabularx}{0.92\textwidth}{>{\centering\arraybackslash}p{3.2cm} X}
\toprule
\textbf{Tầng} & \textbf{Vai trò trong hệ thống} \\
\midrule
Ứng dụng & Frame \texttt{@SEN/@REC} do đề tài định nghĩa \\
...
\begin{center}
\texttt{Ethernet Frame [ IP Packet [ TCP Segment [ @SEN ... \# ] ] ]}
\end{center}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{frame_struct.png}
...
\section{Mô hình TCP Server và TCP Client}
...
\begin{itemize}
    \item \textbf{ESP32:} TCP Server, IP tĩnh \texttt{192.168.2.177}, port \texttt{5000}.
    \item \textbf{Python GUI:} TCP Client, chủ động kết nối tới ESP32.
\end{itemize}
...
\section{Frame truyền thông ứng dụng}
...
\centering
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{0.95\textwidth}{>{\centering\arraybackslash}p{1.2cm} >{\centering\arraybackslash}p{2.5cm} X}
\toprule
\textbf{Byte} & \textbf{Frame gửi} & \textbf{Ý nghĩa} \\
\midrule
...
Frame phản hồi từ ESP32 có cùng độ dài nhưng dùng header \texttt{@REC}. Hai byte vị trí 7 và 8 lần lượt là \texttt{STATUS} và \texttt{ERROR} để báo kết quả xử lý lệnh.
...
\begin{tabularx}{0.95\textwidth}{>{\centering\arraybackslash}p{1.2cm} >{\centering\arraybackslash}p{2.5cm} X}
\toprule
\textbf{Byte} & \textbf{Frame phản hồi} & \textbf{Ý nghĩa} \\
\midrule
0 & @ & Ký tự bắt đầu frame \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{connect.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{w5500.png}
\caption{Sơ đồ kết nối chân của module W5500}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{l298N.png}
\caption{Sơ đồ kết nối chân để kết nối với L298N}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{connect_L298N.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{dc_servo.png}
\caption{ Sơ đồ kết nối chân của DC Servo}
\end{figure}
